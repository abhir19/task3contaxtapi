    ...
    <Fragment>
      <div className="container Articleedit">
        ...
        <form onSubmit={onSubmit} autoComplete="off" style={{width:'90vw',marginLeft:'20px'}}>
                 <label >Title</label>
                <input className="form-control" name="Title" placeholder="Title"   value={selectedArticle.title}
              onChange={e => handleOnChange("title", e.target.value)} required/><br />
                   <label >Length</label>  
                  < input className="form-control" name="Length" placeholder="Length" value={selectedArticle.length}
              onChange={e => handleOnChange("length", e.target.value)} required/><br /> 
